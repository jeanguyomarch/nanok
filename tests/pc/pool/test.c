...
static s_pool _pool = NK_POOL_INIT(_storage);
...
test_pool_reserve(void)
{
   ...
   for (size_t i = 0u; i < 8u; i++)
     {
        data = nk_pool_reserve(&_pool);
        KTEST_ASSERT(data == &_storage[i]);
        KTEST_ASSERT(nk_pool_index_get(&_pool, data) == i);
     }
   ...
   for (size_t i = 0u; i < 100u; i++)
     {
        nk_pool_release(&_pool, data);
        data = nk_pool_reserve(&_pool);
        KTEST_ASSERT(data == &_storage[7]);
        KTEST_ASSERT(nk_pool_index_get(&_pool, data) == 7u);
     }

   /* Release the first element. Then reserve. We should get the first element
    * back. */
   nk_pool_release_nth(&_pool, 0u);
   data = nk_pool_reserve(&_pool);
   KTEST_ASSERT(data == &_storage[0]);
   KTEST_ASSERT(nk_pool_index_get(&_pool, data) == 0u);
}
